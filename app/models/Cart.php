...
class Cart extends AppModel
{
    ...
    {
        return R::getRow("SELECT p.*, pd.* FROM
        product p JOIN product_description pd on p.id = pd.product_id
...
on p.id = pd.product_id WHERE p.id IN ($ids) AND pd.language_id = ?", [$lang['id']]);
